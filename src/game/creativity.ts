...
export function makePostcard(state:GameState):SeasonPostcard {
  return {winter:structuredClone(state.winter),season:state.season,day:state.day,mountainId:state.mountainId,name:resortName(state),guests:state.totalGuestsSeason,reputation:state.reputation,events:state.hostedEvents.filter(e=>e.season===state.season&&e.status==='success').length,town:structuredClone(state.town),style:structuredClone(state.style)}
}
