import { WinterStory } from './WinterStory'
...
export function ResortJournal() {
  ...
  const [tab,setTab] = useState<'winter'|'briefing'|'visits'|'workshop'|'events'|'postcards'>('winter')
  ...
  return <>
    ...
    <dialog ref={ref} onCancel={()=>setOpen(false)} onClose={()=>setOpen(false)} className="resort-dialog">
      ...
      <nav className="my-4 flex flex-wrap gap-2" aria-label="Journal sections">{(['winter','briefing','visits','workshop','events','postcards'] as const).map(t=><button className={`btn ${tab===t?'btn-primary':'btn-ghost'}`} aria-pressed={tab===t} key={t} onClick={()=>setTab(t)}>{{winter:'❄️ Our winter',briefing:'☀️ Morning briefing',visits:'🎿 Guest stories',workshop:'🎨 Customize',events:'🎉 Events',postcards:'📷 Postcards'}[t]}</button>)}</nav>
      ...
      {tab==='winter'?<WinterStory/>:tab==='workshop'?<ResortWorkshop/>:tab==='events'?<HostedEvents/>:tab==='postcards'?<PostcardStudio/>:tab==='briefing'?<div className="space-y-4">
